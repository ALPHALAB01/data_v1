...
export async function onRequestGet({ env }) {
  try {
    const { results } = await env.DB.prepare(
      "SELECT id, time, time_end, content, detail, keywords, amount, client, is_default, hidden_in_history, status, sort_order, created_at FROM history ORDER BY created_at DESC"
    ).all();
...
export async function onRequestPost({ request, env }) {
  try {
    ...
    const res = await env.DB.prepare(
      "INSERT INTO history (time, time_end, content, detail, keywords, amount, client, is_default, hidden_in_history, status, sort_order, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"
    ).bind(startYMD, endYMD, content, detail, keywords, amount, client, isDefault, 0, status, nextSort, now).run();

    return json({
      ok: true,
      item: {
        id: res.meta.last_row_id, time: startYMD, time_end: endYMD, content, detail,
        keywords, amount, client, is_default: isDefault, hidden_in_history: 0, status, sort_order: nextSort, created_at: now
      }
    });
  } catch (e) {
    return json({ ok: false, error: String(e) }, 500);
